...
func TestLanpilotService_Endpoints(t *testing.T) {
	tmpDir := t.TempDir()
	...
	ebpf := lanpilot.NewEbpfCollector()
	arp := lanpilot.NewArpCollector("/dev/null")
	checker := lanpilot.NewInternetChecker(10*time.Second, []lanpilot.TargetConfig{
		{Name: "Target 1", Target: "1.1.1.1:53", Type: lanpilot.ProbeTCP},
	}, db)
	sampler := tsdb.NewSampler(db, ebpf, arp, checker, 1*time.Second)

	svc := NewLanpilotService("eth0", "10.100.0.0/16", ebpf, arp, checker, db, sampler, nil, nil, nil)

	ctx := context.Background()

	// 1. Test GetOverview
	overviewRes, err := svc.GetOverview(ctx, connect.NewRequest(&lanpilotv1.GetOverviewRequest{}))
	if err != nil {
	...
	// 2. Test ListDevices
	devicesRes, err := svc.ListDevices(ctx, connect.NewRequest(&lanpilotv1.ListDevicesRequest{}))
	if err != nil {
	...
	healthRes, err := svc.GetInternetHealth(ctx, connect.NewRequest(&lanpilotv1.GetInternetHealthRequest{}))
	if err != nil {
	...
	tsRes, err := svc.QueryTimeSeries(ctx, connect.NewRequest(&lanpilotv1.QueryTimeSeriesRequest{
		Queries: []*lanpilotv1.TimeSeriesQuery{
...
func TestLanpilotService_DeviceTrafficAndPersistence(t *testing.T) {
	tmpDir := t.TempDir()
	dbPath := filepath.Join(tmpDir, "test.db")
...
`
	...
	ebpf := lanpilot.NewEbpfCollector()
	arp := lanpilot.NewArpCollector(arpPath)
	checker := lanpilot.NewInternetChecker(10*time.Second, []lanpilot.TargetConfig{
		{Name: "Target 1", Target: "1.1.1.1:53", Type: lanpilot.ProbeTCP},
	}, db)
	sampler := tsdb.NewSampler(db, ebpf, arp, checker, 1*time.Second)

	svc := NewLanpilotService("lan", "10.100.0.0/16", ebpf, arp, checker, db, sampler, nil, nil, nil)
	ctx := context.Background()

	res, err := svc.ListDevices(ctx, connect.NewRequest(&lanpilotv1.ListDevicesRequest{}))
	if err != nil {
		t.Fatalf("ListDevices failed: %v", err)
	}
...
type mockDHCPReader struct {
	leases []lanpilot.DHCPLease
}

func (m *mockDHCPReader) GetLeases() ([]lanpilot.DHCPLease, error) {
	return m.leases, nil
}

func (m *mockDHCPReader) GetLeasesMap() (map[string]lanpilot.DHCPLease, map[string]lanpilot.DHCPLease, error) {
	byIP := make(map[string]lanpilot.DHCPLease)
	byMAC := make(map[string]lanpilot.DHCPLease)
...
func TestListDevices_KnownAndUnknownWithDHCP(t *testing.T) {
	...
	ebpf := lanpilot.NewEbpfCollector()
	arp := lanpilot.NewArpCollector(arpPath)
	checker := lanpilot.NewInternetChecker(10*time.Second, nil, db)
	sampler := tsdb.NewSampler(db, ebpf, arp, checker, 1*time.Second)
	...
	svc := NewLanpilotService("lan", "10.100.0.0/16", ebpf, arp, checker, db, sampler, mockDHCP, nil, netMgr)
	res, err := svc.ListDevices(context.Background(), connect.NewRequest(&lanpilotv1.ListDevicesRequest{}))
	if err != nil {
		t.Fatalf("ListDevices failed: %v", err)
	}

	var dKnown, dUnknown *lanpilotv1.Device
...
func TestLanpilotService_WakeOnLan(t *testing.T) {
	tmpDir := t.TempDir()
	dbPath := filepath.Join(tmpDir, "test.db")
	...
	res, err := svc.WakeOnLan(ctx, connect.NewRequest(&lanpilotv1.WakeOnLanRequest{
	...
	resAuto, err := svc.WakeOnLan(ctx, connect.NewRequest(&lanpilotv1.WakeOnLanRequest{
		IpAddr: "10.100.1.88",
	}))
	if err != nil {
	...
	resFail, err := svc.WakeOnLan(ctx, connect.NewRequest(&lanpilotv1.WakeOnLanRequest{
		IpAddr: "10.100.99.99",
	}))
	if err != nil {
		t.Fatalf("WakeOnLan unexpected RPC error: %v", err)
	...
	resInvalid, err := svc.WakeOnLan(ctx, connect.NewRequest(&lanpilotv1.WakeOnLanRequest{
...
func TestLanpilotService_DDNS(t *testing.T) {
	...
	svcNoDDNS := NewLanpilotService("lan", "10.100.0.0/16", nil, nil, nil, db, nil, nil, nil, nil)
	stNoDDNS, err := svcNoDDNS.GetDDNSStatus(ctx, connect.NewRequest(&lanpilotv1.GetDDNSStatusRequest{}))
	if err != nil {
	...
	syncNoDDNS, err := svcNoDDNS.SyncDDNS(ctx, connect.NewRequest(&lanpilotv1.SyncDDNSRequest{}))
	if err != nil {
	...
	svc := NewLanpilotService("lan", "10.100.0.0/16", nil, nil, nil, db, nil, nil, mgr, nil)

	// Sync via RPC
	syncRes, err := svc.SyncDDNS(ctx, connect.NewRequest(&lanpilotv1.SyncDDNSRequest{Force: true}))
	if err != nil {
	...
	// Fetch status
	stRes, err := svc.GetDDNSStatus(ctx, connect.NewRequest(&lanpilotv1.GetDDNSStatusRequest{}))
	if err != nil {
...
func TestLanpilotService_NetworkMgr(t *testing.T) {
	tmpDir := t.TempDir()
	...
	svc := NewLanpilotService("lan", "10.100.0.0/16", nil, nil, nil, db, nil, nil, nil, nm)
	ctx := context.Background()

	// 1. UpsertConfigDevice
	upsertDevRes, err := svc.UpsertConfigDevice(ctx, connect.NewRequest(&lanpilotv1.UpsertConfigDeviceRequest{
	...
	listDevRes, err := svc.ListDevices(ctx, connect.NewRequest(&lanpilotv1.ListDevicesRequest{}))
	if err != nil {
		t.Fatalf("ListDevices failed: %v", err)
	}
	...
	upsertDnsRes, err := svc.UpsertConfigDnsRecord(ctx, connect.NewRequest(&lanpilotv1.UpsertConfigDnsRecordRequest{
	...
	listDnsRes, err := svc.ListConfigDnsRecords(ctx, connect.NewRequest(&lanpilotv1.ListConfigDnsRecordsRequest{}))
	if err != nil {
	...
	delDnsRes, err := svc.DeleteConfigDnsRecord(ctx, connect.NewRequest(&lanpilotv1.DeleteConfigDnsRecordRequest{
		Name: "photos",
	}))
	if err != nil {
	...
	delDevRes, err := svc.DeleteConfigDevice(ctx, connect.NewRequest(&lanpilotv1.DeleteConfigDeviceRequest{
		Id: "test-apple-tv",
	}))
	if err != nil {
	...
	listDevResAfter, _ := svc.ListDevices(ctx, connect.NewRequest(&lanpilotv1.ListDevicesRequest{}))
