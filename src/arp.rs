...
fn get_mac_through_arp(interface: NetworkInterface, target_ip: Ipv4Addr) -> Result<()> {
    ...
    //     println!("Received reply: {} {}", arp.get_sender_proto_addr(), arp.get_target_hw_addr());

    //     if arp.get_sender_proto_addr() == target_ip && arp.get_target_hw_addr() == interface.mac.unwrap() {
...
pub fn main() {
    ...
    let iface_name = match args.next() {
        Some(n) => n,
        None => {
            writeln!(io::stderr(), "USAGE: arp_packet <NETWORK INTERFACE> <TARGET IP>").unwrap();
            process::exit(1);
        }
    };
    ...
    get_mac_through_arp(interface, target_ip.unwrap());
