...
use super::args::ScanOptions;
use super::args::ScanTiming;
use super::utils;
...
pub fn compute_scan_estimation(host_count: u128, options: &Arc<ScanOptions>) -> ScanEstimation {
    ...
    ScanEstimation { interval_ms, duration_ms, request_size, bandwidth }
}
...
pub fn send_arp_request(
    ...
    let mut ethernet_packet = MutableEthernetPacket::new(&mut ethernet_buffer).context("Could not build Ethernet packet")?;
    ...
    let source_mac = match options.source_mac {
        ...
        None => interface.mac.context("interface should have mac address")?,
    };
    ...
    let mut arp_packet = MutableArpPacket::new(&mut arp_buffer).context("Could not build ARP packet")?;
    ...
    if let Some(vlan_id) = options.vlan_id {
        ...
        let mut vlan_packet = MutableVlanPacket::new(&mut vlan_buffer).context("Could not build VLAN packet")?;
...
impl NetworkIterator {
    pub fn new(networks_ref: &[&IpNetwork], is_random: bool) -> NetworkIterator {
        ...
        NetworkIterator { current_iterator: None, networks, is_random, random_pool: vec![] }
    }
...
pub fn receive_arp_responses(
    ...
    loop {
        ...
        if let Some(arp) = arp_packet {
            ...
            discover_map.insert(sender_ipv4, TargetDetails { ipv4: sender_ipv4, mac: sender_mac, hostname: None, vendor: None });
        }
    }
    ...
    let response_summary = ResponseSummary { packet_count, arp_count, duration_ms: start_recording.elapsed().as_millis() };
