...
label_replace(
  sum by(${ipLabel}) (
    ${queryFunc}(
      lanpilot_bytes_total{${labels},instance=~"$instance"}[${queryType}]
    ) > 0
  ),
  "ip_addr", "$1", "${ipLabel}", "(.*)"
)` +
...
* on(ip_addr) group_left(hw_addr, device, hostname) (${queryType === '$__range' ? 'keep_last_value' : ''}(
    sum by (ip_addr, hw_addr, device, hostname) (lanpilot_arp_devices{instance=~"$instance"})
  ) * 0 + 1)
...
const panels: PanelRowAndGroups = [
  NewPanelGroup({ title: 'Overview' }, [
    NewPanelRow({ datasource, height: 3 }, [
      NewStatPanel({
        title: 'Internet',
        targets: [{ expr: 'max(lanpilot_internet_connection_is_up{instance=~"$instance"})' }],
        unit: units.Short,
      ...
      NewStatPanel({
        ...
        targets: [{ expr: '(1 - avg_over_time(max(lanpilot_internet_connection_is_up{instance=~"$instance"})[$__range])) * $__range_s' }],
        thresholds: {
          mode: dashboard.ThresholdsMode.Absolute,
          steps: [
      ...
      NewStatPanel({
        ...
        targets: [
          {
            expr: 'avg(rate(lanpilot_internet_connection_duration_seconds_sum{instance=~"$instance"}[$__rate_interval]) / rate(lanpilot_internet_connection_duration_seconds_count{instance=~"$instance"}[$__rate_interval]))',
          },
        ],
      ...
      NewStatPanel({
        ...
        targets: [{ expr: 'histogram_quantile(0.99, sum by (le) (rate(lanpilot_internet_connection_duration_seconds_bucket{instance=~"$instance"}[$__rate_interval])))' }],
        unit: units.Seconds,
        reduceCalc: 'max',
        thresholds: {
          mode: dashboard.ThresholdsMode.Absolute,
      ...
      NewStatPanel({
        ...
        targets: [{ expr: 'count(last_over_time((lanpilot_arp_devices{instance=~"$instance"} == 2)[$__range]))' }],
        unit: units.Short,
      }),
      NewBarGaugePanel(
        {
        ...
        { expr: 'sum by(src) (increase(lanpilot_bytes_total{dst=~"$localips",src=~"internet",instance=~"$instance"}[$__range]))', legendFormat: 'Download' },
        { expr: 'sum by(dst) (increase(lanpilot_bytes_total{src=~"$localips",dst=~"internet",instance=~"$instance"}[$__range]))', legendFormat: 'Upload' }
      ),
    ]),
  ]),
  NewPanelRow({ datasource, height: 6 }, [
    NewTimeSeriesPanel({
      ...
      targets: [
        {
          expr: 'rate(lanpilot_internet_connection_duration_seconds_sum{instance=~"$instance"}[$__rate_interval]) / rate(lanpilot_internet_connection_duration_seconds_count{instance=~"$instance"}[$__rate_interval])',
          legendFormat: '{{ addr }} average',
        },
        {
          expr: '1 - max(lanpilot_internet_connection_is_up{instance=~"$instance"})',
          legendFormat: 'down',
        },
        // {
        //   expr: 'histogram_quantile(0.99, sum by (le) (rate(lanpilot_internet_connection_duration_seconds_bucket{instance=~"$instance"}[$__rate_interval])))',
        //   legendFormat: '99p',
        // },
        {
          expr: 'histogram_quantile(0.95, sum by (le) (rate(lanpilot_internet_connection_duration_seconds_bucket{instance=~"$instance"}[$__rate_interval])))',
          legendFormat: '95p',
        },
        {
          expr: 'histogram_quantile(0.50, sum by (le) (rate(lanpilot_internet_connection_duration_seconds_bucket{instance=~"$instance"}[$__rate_interval])))',
          legendFormat: '50p',
        },
      ],
      legendCalcs: [],
    ...
    NewTimeSeriesPanel({
      ...
      targets: [
        {
          expr: '1 - lanpilot_internet_connection_is_up{instance=~"$instance"}',
  ...
  NewPanelRow({ datasource, height: 12 }, [
    NewTablePanel({
      ...
      targets: [
        {
          expr: 'sum by (ip_addr, hw_addr, hostname, device) (lanpilot_arp_devices{instance=~"$instance"})',
          // expr: 'sum by (ip_addr, hw_addr, device) (last_over_time(lanpilot_arp_devices{instance=~"$instance"}[$__range])) * on(ip_addr) group_left(hostname) max by (ip_addr, hostname) (last_over_time(lanpilot_hostnames{instance=~"$instance"}[$__range]))',
          format: 'table',
          type: 'instant',
          refId: 'DEVICE',
        },
        {
          expr: 'label_move(sum by (dst) (increase(lanpilot_bytes_total{dst=~"$localips",src=~"internet",instance=~"$instance"}[$__range]) > 0), "dst", "ip_addr")',
          format: 'table',
          type: 'instant',
          refId: 'INGRESS_BYTES',
        },
        {
          expr: 'label_move(sum by (dst) (increase(lanpilot_packets_total{dst=~"$localips",src=~"internet",instance=~"$instance"}[$__range]) > 0), "dst", "ip_addr")',
          format: 'table',
          type: 'instant',
          refId: 'INGRESS_PACKETS',
        },
        {
          expr: 'label_move(sum by (src) (increase(lanpilot_bytes_total{src=~"$localips",dst=~"internet",instance=~"$instance"}[$__range]) > 0), "src", "ip_addr")',
          format: 'table',
          type: 'instant',
          refId: 'EGRESS_BYTES',
        },
        {
          expr: 'label_move(sum by (src) (increase(lanpilot_packets_total{src=~"$localips",dst=~"internet",instance=~"$instance"}[$__range]) > 0), "src", "ip_addr")',
          format: 'table',
          type: 'instant',
...
const lanpilotDashboard = newDashboard({
  title: 'Lanpilot',
  ...
  variables: [
    ...
    NewQueryVariable({ datasource, name: 'localips', label: 'Local IPs', query: 'label_values(lanpilot_packets_total, dst)', multi: true, includeAll: true }),
    NewQueryVariable({ datasource, name: 'instance', label: 'Instance', query: 'label_values(lanpilot_internet_connection_is_up, instance)' }),
  ],
})

writeDashboardAndPostToGrafana({
  dashboard: lanpilotDashboard.build(),
  filename: 'lanpilot-dashboard.json',
})
